...
import com.centimia.orm.ezqu.CascadeType;
...
public @interface One2Many {
	...
	/**
	 * <b>Relevant only when using a relation table, since when using a Foreign Key the relation is maintained in the<br>child table and ezQu knows this from the child type</b><br>
	 * The name of the column representing the other side of the relation. If not present EzQu will use the simple java class name of the child entity.
	 */
	 ...
	 * If left empty ezqu will assume that either there is a field on the other object which has a name identical to 'this' tableName or there is a column in the Db with 'this' tableName<p>
	 ...
	 * <li>In a single sided relationship, When not using a relationship table, a column FK must exist in the child table,<br>even if it does not exist in the object. In such a case instead of a field name one should write the column-name.</li>
	 * </ol></p> 
	 */
	 ...
	 * If not named EzQu will attempt to find it, or fail to set up the relation table.
	 */
