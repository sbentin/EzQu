 ...
 * @author Shai Bentin
 */
public final class EzquSessionFactory {
	 ...
	 * When wrapping EzQu work in a transaction autoCommit=true usually has no effect
	 * however setting it to false is recommended. Default is 'true'.
	 */
	private boolean autoCommit = true;

	/** When true EzQu will output to log the sql statements it produces */
	...
	/** If set to false, EzQu will not attempt to create the table from the object and assume it exists */
	 ...
	 * INTERNAL  - to let EzquSessionFactory configure your connections via autoCommit and transactionIsolation fields. This means
	 ...
	 */
	public EzquSessionFactory(CommonDataSource ds) {
    	if (null == ds)
    		throw new EzquError("IllegalState - Missing valid datasource!!!");
	...
	/**
	 * Start ezqu with or without a transaction manager but change some basic JDBC transaction defaults allowing manual commit/ rollback handeling.
	 ...
	 */
	public EzquSessionFactory(CommonDataSource ds, boolean autoCommit, int transactionIsolation) {
		this(ds);
	 ...
	 * @param isExternal
	 */
	public EzquSessionFactory setAcidConfigIsExternal(boolean isExternal) {
		if (isExternal)
     ...
     * @param createTable
     */
    public EzquSessionFactory setCreateTable(boolean createTable) {
     ...
     * @param dialect
     */
    public EzquSessionFactory setDialect(Dialect dialect) {
	 ...
	 */
	public EzquSessionFactory setShowSQL(boolean showSQL) {
	...
	<T,X> X getPrimaryKey(T t) {
    	 ...
    	 if (def == null && isCreateTable()) {
    		 // this means that it is expected of the framework to create this table
    		 // and we have not accessed it yet so we define it now.
    		 def = updateTableDefinition(t.getClass(), currentSession(), true);
    	 }
    	 if (def == null)
