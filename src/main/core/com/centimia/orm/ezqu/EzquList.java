...
/**
 * A List implementation of the Ezqu Collection. This list should not be invoked by a User it is invoked by the framework only
 *
 * see AbstractEzquCollection
 * @author Shai Bentin
 */
class EzquList<E> extends AbstractEzquCollection<E> implements List<E> {

	private static final long	serialVersionUID	= -4977190877509050721L;
	protected transient int modCount = 0;
	
	public EzquList(List<E> origList, Db db, FieldDefinition definition, Object parentPk) {
	 ...
	 * (although I don't see it's practicality) is only possible when working inside a Ezqu Session.
	 */
	@Override
	...
	public boolean addAll(int index, Collection<? extends E> c) {
		...
		if (!dbClosed()) {
			...
			result = ((List<E>)originalList).addAll(index, attachedList);
		}
		else
	...
	/**
	 * Getting an object from the list from a specific position may produce an incorrect result since when adding elements to the list outside the ezqu
	 ...
	 * outside the Ezqu Session is not permitted. To go over the list of Items use the Iterator instead.
	 */
	@Override
	...
	/**
	 * Check the index of an item in a the list may produce an incorrect result since when adding elements to the list outside the ezqu
	 ...
	 * outside the Ezqu Session is not permitted.
	 */
	@Override
	...
	public ListIterator<E> listIterator() {
		return new EzquListIterator(((List<E>)originalList).listIterator());
	}

	@Override
	public ListIterator<E> listIterator(int index) {
		return new EzquListIterator(((List<E>)originalList).listIterator(index));
	}
	...
	public E remove(int index) {
		...
		if (!dbClosed()) {
			...
			if (null == internalDeleteMapping)
				internalDeleteMapping = new HashMap<>();
	...
	class EzquListIterator extends EzquIterator implements ListIterator<E> {
		// first index is 0
		int lastIdx = -1;

		EzquListIterator(ListIterator<E> iter) {
			super(iter);
		}
	...
	void merge() {
		super.merge();
		originalList = originalList.stream().map(e -> db.get().checkSession(e)).toList();
	}

	class SubList implements List<E> {
	    private static final String INDEX_D_SIZE_D = "Index: %d, Size: %d";
		private EzquList<E> l;
	    private int offset;
	    private int size;
	    private int expectedModCount;

	    SubList(EzquList<E> list, int fromIndex, int toIndex) {
	        if (fromIndex < 0)
	    ...
	    /**
	     * Unsupported in EzquList.subList because ezquList is actually a special wrapper
	     * @throws EzquError
	     */
	    @Override
		public List<E> subList(int fromIndex, int toIndex) {
	    	throw new EzquError("IllegalState - This list is backed up by the DB. Can't get position of object outside the Db Session");
	    }
